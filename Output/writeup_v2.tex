...
The main findings are: (i) Treasury yields respond approximately 0.5 basis points per 1 basis point surprise, validating identification; (ii) FX responses are heterogeneous but imprecisely estimated ($R^2 < 0.01$); (iii) NFA/GDP does not robustly mediate FX responses in any specification. These null results are informative: they discipline the mechanism by ruling out large, detectable NFA effects in G10 daily data.
...
Table~\ref{tab:treasury_response} reports results. A 1 basis point STMT surprise raises the 2-year yield by approximately 0.5 basis points ($R^2 \approx 0.16$). The response declines monotonically with maturity, consistent with monetary surprises primarily shifting near-horizon expected policy rates. Figure~\ref{fig:treasury_coefficients} visualizes this term-structure pattern: the declining response with maturity is the expected signature of conventional transmission.
...
Table~\ref{tab:nfa_panel} presents the main results. The interaction coefficient $\hat{\beta}_2 = -0.005$ is \textit{negative}---opposite to the balance-sheet hypothesis---and statistically indistinguishable from zero under date-clustered inference ($p = 0.477$).
...
\textbf{Economic interpretation}: For STMT, the NFA slope shifts from $-0.015$ (pre-GFC) to $+0.007$ (post-GFC). This directional movement is \textit{toward} the Antolín-Díaz prediction ($\beta_2 > 0$), but the change is statistically insignificant. With NFA SD $\approx 70$ percentage points:
\begin{itemize}
...
\begin{enumerate}
    \item \textbf{Strong domestic transmission}: Treasury yields respond significantly to surprises ($\sim$0.5 bp per 1 bp shock), validating identification.
...
These null results are informative, not discouraging. They rule out large NFA-mediated effects in G10 daily data and highlight the empirical challenges of detecting balance-sheet channels at this frequency. Extensions using intraday data, gross position measures, or expanded currency samples (including emerging markets with higher dollar debt shares) may yield sharper identification.

The findings do not reject the theoretical relevance of external balance sheets for monetary policy transmission. Rather, they indicate that within this specific framework---G10 currencies, daily frequency, net positions---the balance-sheet channel is either small, offset by other forces, or operative through channels not captured by NFA/GDP.
...
\midrule
STMT (bps) & 269 & 0.00 & 3.68 & $-26.4$ & 0.46 & 9.01 \\
MP1 (bps) & 269 & $-1.04$ & 6.80 & $-46.5$ & 0.00 & 16.3 \\
MP2 (bps) & 269 & $-0.78$ & 4.71 & $-39.0$ & 0.00 & 15.1 \\
\bottomrule
\end{tabular}
\begin{flushleft}
\footnotesize \textit{Notes}: Sample covers 269 FOMC announcements, January 1994 to December 2024. STMT is the statement/PC surprise; MP1 is the target rate surprise; MP2 is the path factor surprise. All measured in basis points.
\end{flushleft}
\end{table}
...
\midrule
2-year & 0.478*** & 0.163 & 0.356*** & 0.142 \\
5-year & 0.412*** & 0.152 & 0.385*** & 0.178 \\
10-year & 0.287*** & 0.112 & 0.298*** & 0.098 \\
\bottomrule
\end{tabular}
\begin{flushleft}
\footnotesize \textit{Notes}: OLS regression of Treasury yield change (bps) on monetary surprise (bps) on FOMC announcement days. Heteroskedasticity-robust standard errors. *** $p<0.01$.
\end{flushleft}
\end{table}
...
\end{tabular}
\begin{flushleft}
\footnotesize \textit{Notes}: Dependent variable is spot return $r_{i,t} = -\Delta\log(e_{i,t})$; positive = foreign appreciation. Heteroskedasticity-robust SEs. 269 FOMC events.
\end{flushleft}
\end{table}
...
\end{tabular}
\begin{flushleft}
\footnotesize \textit{Notes}: Dependent variable is spot return $r_{i,t} = -\Delta\log(e_{i,t})$; positive = foreign appreciation. NFA/GDP is demeaned so $\beta_1$ is the effect at average NFA. Standard errors in parentheses. * $p<0.10$, ** $p<0.05$, *** $p<0.01$.
\end{flushleft}
\end{table}
